...
func (b *JJBackend) ListWorkspaces() ([]vcs.Worktree, error) {
...
func (b *JJBackend) PruneWorktrees(repoDir string) error {
	...
	for _, ws := range workspaces {
		...
		path, pathErr := getWorkspacePath(repoDir, ws.Branch)
		if pathErr != nil {
			continue
		}
		if _, statErr := os.Stat(path); os.IsNotExist(statErr) {
			cmd := exec.Command("jj", "workspace", "forget", ws.Branch, "-R", repoDir)
			_ = cmd.Run()
		}
	}
	return nil
...
func (b *JJBackend) GetWorktreeForBranch(branchName string) (string, error) {
	worktrees, err := b.ListWorkspaces()
	if err != nil {
		return "", err
	}
