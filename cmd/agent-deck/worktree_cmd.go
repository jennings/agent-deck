...
func handleWorktreeList(profile string, args []string) {
	...
	backend, err := git.NewGitBackend(cwd)
	if err != nil {
...
func handleWorktreeCleanup(profile string, args []string) {
	...
	var cleanupBackend *git.GitBackend

	if cleanupB, bErr := git.NewGitBackend(cwd); bErr == nil {
	...
	for _, wt := range orphanedWorktrees {
		if cleanupBackend == nil {
			fmt.Fprintf(os.Stderr, "Warning: no git backend for worktree removal\n")
			break
		}
...
func handleWorktreeFinish(profile string, args []string) {
	...
	finishBackend, err := git.NewGitBackend(repoRoot)
	if err != nil {
		out.Error(fmt.Sprintf("failed to initialize git: %v", err), ErrCodeInvalidOperation)
		os.Exit(1)
	}
